...
- 每 200ms 扫描本地网卡 IP 变化；每次公网 IP 检查完成 3 秒后再次检查，IP 变化后立即确认是否仍在允许国家/地区列表，不符合或无法确认时强制断开 Windows 网络
...
右键托盘图标可选择“屏蔽网络检查 3/5/10 分钟”。未屏蔽且网络检查开启时，程序会每 200ms 扫描本地网卡 IP 指纹；本地 IP 变化会立即触发公网校验。公网校验通过多个 IP 定位服务交叉确认当前 IP，只有所有成功结果都在允许列表中才继续联网。检查站点按 round-robin 轮换，每轮拿到两个成功结果就停止，降低单个站点访问频率。
...
| `network_check.providers` | 公网 IP 检查站点列表，默认全部：`ipinfo`、`ifconfig`、`ip-api`、`ipapi`、`ipwhois`，开启检查时至少需要 2 个 |
